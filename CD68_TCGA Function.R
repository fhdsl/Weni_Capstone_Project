MacroExpr_Comparison = function(genename){row_info <- as.data.frame(rowData(data_se))
...
target_row <- rownames(row_info[row_info$gene_name == "CD68" , ])
gene_expr2 <- expression[target_row, ]
clean_df$CD68 <- as.numeric(gene_expr2[match(clean_df$barcode, names(gene_expr2))])
clean_df2 = select(clean_df, gene_name_group, genename, CD68)
clean_df2 = pivot_longer(clean_df2, c(genename, "CD68"), names_to = "Genes", values_to = "Expression")
...
Macrocombined_boxplots <- map(GeneNames, MacroExpr_Comparison)


destination = "C:\\Users\\weniw\\Downloads\\CD68combined_boxplots.pdf"
pdf(file = destination)
par(mfrow = c(2,2))
Macrocombined_boxplots
dev.off()