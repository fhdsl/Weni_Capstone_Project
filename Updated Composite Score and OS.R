compositeSurvivalAnalysis = function(summarized_experiment, normalized_expression) {
  ...
  rownames(normalized_expression) <- rowData(summarized_experiment)$gene_name
  sum(duplicated(rownames(normalized_expression)))
  
  rankData <- rankGenes(normalized_expression)
  ...
  fit <- surv_fit(as.formula("Surv(time, status) ~ scoregroup"), data = clean_clinicalsing1)
  ...
  return_list = list(plot=plot, p_value = pvalue_result)
